...
export class ExpenseCategoriesComponent implements OnInit {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err: { error?: { detail?: string } }) => {
          this.loading.set(false);
          this.notification.error(err?.error?.detail ?? 'Failed to load expense categories.');
        }
      });
  }
  ...
  startEdit(item: ExpenseCategoryDto): void {
  ...
  save(): void {
    ...
    const body = { name: value.name.trim(), isActive: value.isActive };
    const editingId = this.editingId();
    this.loading.set(true);

    const request$ = editingId
      ? this.api.updateExpenseCategory(editingId, body)
    ...
    request$.subscribe({
      next: () => {
        ...
        this.cancelEdit();
        this.reload();
        this.notification.success(editingId ? 'Category updated.' : 'Category created.');
      },
      error: (err: { error?: { detail?: string } }) => {
        this.loading.set(false);
        this.notification.error(err?.error?.detail ?? 'Failed to save expense category.');
      }
    });
  }

  remove(item: ExpenseCategoryDto): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean | undefined) => {
      ...
      this.api.deleteExpenseCategory(item.id).subscribe({
        next: () => {
        ...
        error: (err: { error?: { detail?: string } }) =>
          this.notification.error(err?.error?.detail ?? 'Failed to delete expense category.')
      });
    });
  }
