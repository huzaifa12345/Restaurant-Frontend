...
export class EmployeesComponent implements OnInit {
  ...
  readonly displayedColumns = ['name', 'phone', 'designation', 'dailyWage', 'monthlySalary', 'active', 'actions'];

  @ViewChild('employeeFormTemplate') private readonly employeeFormTemplate?: TemplateRef<unknown>;
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err: { error?: { detail?: string } }) => {
          this.loading.set(false);
          this.notification.error(err?.error?.detail ?? 'Failed to load employees.');
        }
      });
  }
  ...
  startEdit(item: EmployeeDto): void {
    ...
    this.form.reset({
      ...
      dailyWage: item.dailyWage ?? null,
      monthlySalary: item.monthlySalary ?? null,
  ...
  save(): void {
    ...
    const request$ = editingId
      ? this.api.updateEmployee(editingId, body)
    ...
    request$.subscribe({
      next: () => {
        ...
        this.cancelEdit();
        this.reload();
        this.notification.success(editingId ? 'Employee updated.' : 'Employee created.');
      },
      error: (err: { error?: { detail?: string } }) => {
        this.loading.set(false);
        this.notification.error(err?.error?.detail ?? 'Failed to save employee.');
      }
    });
  }

  remove(item: EmployeeDto): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean | undefined) => {
      ...
      this.api.deleteEmployee(item.id).subscribe({
        ...
        error: (err: { error?: { detail?: string } }) =>
          this.notification.error(err?.error?.detail ?? 'Failed to delete employee.')
      });
    });
  }

  private openFormDialog(): void {
    if (!this.employeeFormTemplate) {
      return;
    }

    this.activeDialogRef?.close();
    this.activeDialogRef = this.dialog.open(this.employeeFormTemplate, {
      width: '640px',
      autoFocus: false
    });
